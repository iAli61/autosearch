...
class SimpleTextBrowser:
    ...
    def set_address(self, uri_or_path: str) -> None:
        ...
        elif uri_or_path.startswith("bing News:"):
            self._bing_news_search(uri_or_path[len("bing News:") :].strip())
        else:
    ...
    def _bing_api_call(self, query: str, bing_search_type: str) -> Dict[str, Dict[str, List[Dict[str, Union[str, Dict[str, str]]]]]]:
    ...
    def _bing_news_search(self, query: str) -> None:
        results = self._bing_api_call(query, "News")

        news_snippets = []
        
        news_snippets.extend(
            f"{idx}. {page['datePublished'][:-9]}: [{page['name']}]({page['url']})\n{page['description']}"
            for idx, page in enumerate(results["value"], start=1)
        )
        self.page_title = f"{query} - Search"

        if news_snippets:
            content = (
                f"A Bing News search for '{query}' found {len(news_snippets)} results:\n\n## News Results\n"
                + "\n\n".join(news_snippets)
            )
        else:
            content = f"A Bing News search for '{query}' found no results. Please refine your search query."
