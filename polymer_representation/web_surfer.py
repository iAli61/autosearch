...
class WebSurferAgent(ConversableAgent):
    ...
    def _register_functions(self) -> None:
        ...
        @self._user_proxy.register_for_execution()
        @self._assistant.register_for_llm(
            name="informational_news_search",
            description="Perform an INFORMATIONAL News search query then return the search results.",
        )
        def _informational_news_search(query: Annotated[str, "The informational News search query to perform."]) -> str:
            self.browser.visit_page(f"bing News: {query}")
            header, content = _browser_state()
            return header.strip() + "\n=======================\n" + content
